...
export default function AddScheduleDialogForm({
  headerTitle,
  visible,
  setVisible,
  ...
  const { mutate } = useCreateScheduleMutation<IApiError>(GRAPHQL_CLIENT, {
    onSuccess: () => {
      toast.current?.show({
        severity: 'success',
  ...
  const onSubmit: SubmitHandler<IUpsertScheduleInput> = (data: IUpsertScheduleInput) => {
