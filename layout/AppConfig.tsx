...
const AppConfig = (props: AppConfigProps) => {
  ...
  const { layoutConfig, setLayoutConfig } = useGlobalAppStore();
  const contextPath = '';
  ...
  const changeInputStyle = (e: RadioButtonChangeEvent) => {
    setLayoutConfig({ ...layoutConfig, inputStyle: e.value });
  };

  const changeRipple = (e: InputSwitchChangeEvent) => {
    PrimeReact.ripple = e.value as boolean;
    setLayoutConfig({ ...layoutConfig, ripple: e.value as boolean });
  };

  const changeMenuMode = (e: RadioButtonChangeEvent) => {
    setLayoutConfig({ ...layoutConfig, menuMode: e.value });
  };
  ...
  const changeTheme = (theme: string, colorScheme: string) => {
    ...
    replaceLink(themeLink, newHref, () => {
      setLayoutConfig({ ...layoutConfig, theme, colorScheme });
    });
  };
  ...
  const decrementScale = () => {
    setLayoutConfig({ ...layoutConfig, scale: layoutConfig.scale - 1 });
  };
